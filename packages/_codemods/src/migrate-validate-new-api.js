...
const files = gatherFilesFromDir(
  pathLib.resolve(__dirname, './example-sources/migrate-validate-new-api'),
);
  ...
  babel.traverse(ast, {
    ...
    Program: {
      exit(path) {
        ...
        /**
         * @desc Go from something like `minLengthValidator(3)` to `new MinLength(3, {type: 'info'}`)
         *
         * @param {object} callExpressionPath CallExpression path
         * @param {'error'|'warning'|'info'|'success'} type
         */
        function replaceValidatorsInCallExpression(callExpressionPath, type, foundValidators) {
          if (!foundValidators.includes(callExpressionPath.node.callee.name)) {
            return;
          }

          const oldName = callExpressionPath.node.callee.name;
          const oldArgs = callExpressionPath.node.arguments;
          ...
          if (type === 'error' && oldArgs[1]) {
            ...
            if (!oldArgs[1]) {
              ...
              } else if (callExpressionPath.node.arguments[1].type === 'Identifier') {
                // const binding = callExpressionPath.scope.getBinding(callExpressionPath.node.arguments[1].name);
                // console.log('binding', binding);
                // const objectExpressionNode = binding.callExpressionPath.node.init;
                const { name } = callExpressionPath.node.arguments[1];
          ...
          callExpressionPath.replaceWith(t.newExpression(t.Identifier(newName), newArgs));
        }
              ...
              literal.quasis.forEach((quasi, index) => {
                ...
                if (quasi.value.raw.replace('\\"', '"').endsWith(`.${prop}="`)) {
                  ...
                  propPath[index].traverse({
                    CallExpression: path =>
                      replaceValidatorsInCallExpression(path, type, foundValidators),
                  });
                }
              });
            });
        ...
        const allPromises = Promise.all(
          arr.map(async ([tpLiteralPath, props]) => {
