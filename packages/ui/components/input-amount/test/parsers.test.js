...
describe('parseAmount()', async () => {
  ...
  it('parses based on locale when "preformatted" mode used', async () => {
    expect(parseAmount('123,456.78', { mode: 'auto' })).to.equal(123456.78);
    expect(parseAmount('123,456.78', { mode: 'preformatted' })).to.equal(123456.78);
    expect(parseAmount('123.456,78', { mode: 'auto' })).to.equal(123456.78);
    expect(parseAmount('123.456,78', { mode: 'preformatted' })).to.equal(123.45678);
  });
});
