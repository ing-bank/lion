...
export function runInputStepperSuite(klass = LionInputStepper) {
  ...
  const defaultInputStepper = html`
    <${tag} name="year" label="Years"></${tag}>
  `;
  const inputStepperWithAttrs = html`<${tag}
    step="10"
    min="100"
    max="200"
    label="Label"
  ...
  describe('InputStepper', async () => {
    ...
    describe('Formatter', () => {
      ...
      it('formatNumber uses locale provided in formatOptions', async () => {
        let el = await fixture(html`
          <${tag} .formatOptions="${{ locale: 'en-GB' }}" .modelValue="${1234.56}"
          ></${tag}>
        `);
        expect(el.formattedValue).to.equal('1,234.56');
        el = await fixture(html`
          <${tag} .formatOptions="${{ locale: 'nl-NL' }}" .modelValue="${1234.56}"
          ></${tag}>
        `);
        expect(el.formattedValue).to.equal('1.234,56');
      });
    ...
    describe('User interaction', () => {
      ...
      describe('It align to steps', () => {
        it('aligns the value to the nearest step when incrementing', async () => {
          let el = await fixture(html`<${tag} step="10" min="0" max="100" value="55"></${tag}>`);
          ...
          // min 1
          el = await fixture(html`<${tag} step="10" min="1" max="100" value="55"></${tag}>`);
          ...
          // min 34
          el = await fixture(html`<${tag} step="10" min="34" max="100" value="55"></${tag}>`);
          incrementButton = el.querySelector('[slot=suffix]');
          incrementButton?.dispatchEvent(new Event('click'));
          await el.updateComplete;
          ...
          // min -23
          el = await fixture(html`<${tag} step="10" min="-23" max="100" value="55"></${tag}>`);
          incrementButton = el.querySelector('[slot=suffix]');
          incrementButton?.dispatchEvent(new Event('click'));
          await el.updateComplete;
          ...
          // min -23
          el = await fixture(html`<${tag} step="10" min="-23" max="100" value="-9"></${tag}>`);
          incrementButton = el.querySelector('[slot=suffix]');
          incrementButton?.dispatchEvent(new Event('click'));
          await el.updateComplete;
        ...
        it('handles decimal step values correctly', async () => {
          ...
          let el = await fixture(html`<${tag} step="0.1" min="0" max="9" value="5.55"></${tag}>`);
          ...
          el = await fixture(html`<${tag} step="0.1" min="0" max="9" value="3.27"></${tag}>`);
    ...
    describe('Accessibility', () => {
      ...
      it('is accessible when disabled', async () => {
        const el = await fixture(html`<${tag} label="rsvp" disabled></${tag}>`);
        await expect(el).to.be.accessible();
      });
      ...
      it('can give aria-valuetext to override default value as a human-readable text alternative', async () => {
        ...
        const el = await fixture(html`
          <${tag} min="1" max="3" .valueTextMapping="${values}">></${tag}>
        `);
        el.modelValue = 1;
        await el.updateComplete;
        expect(el._inputNode.hasAttribute('aria-valuetext')).to.be.true;
