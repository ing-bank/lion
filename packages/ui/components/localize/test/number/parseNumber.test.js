...
describe('parseNumber()', () => {
  ...
  it('detects separators withLocale when "preformatted" mode used e.g. 123.456,78', async () => {
    expect(parseNumber('123,456.78', { mode: 'auto' })).to.equal(123456.78);
    expect(parseNumber('123,456.78', { mode: 'preformatted' })).to.equal(123456.78);
    expect(parseNumber('123.456,78', { mode: 'auto' })).to.equal(123456.78);
    expect(parseNumber('123.456,78', { mode: 'preformatted' })).to.equal(123.45678);
  });
