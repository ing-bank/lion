...
import { preprocessorFactory } from '../src/utils/preprocessorFactory.js';
 ...
 * @param {?} newViewValue
 */
function mimicUserInput(formControl, newViewValue, { caretIndex = null } = {}) {
  ...
  describe('FormatMixin', async () => {
    ...
    describe('Parser / formatter / serializer / preprocessor', () => {
      ...
      describe('Preprocessor', () => {
        ...
        it('returns {viewValue: string, caretIndex:number}', async () => {});

        it.only('supports input masks', async () => {
          const preprocessorFunction = preprocessorFactory('___-____-_____', {
            // restrict to just digits
            restrictCondition: (/** @type {string} */ c) => !Number.isNaN(Number(c)),
          });

          const preprocessorSpy = sinon.spy(preprocessorFunction);
          ...
          `));
          const caretIndex = 9;
          mimicUserInput(el, '1w23456789', { caretIndex });
          expect(el.value).to.equal('123-4567-89___');
          expect(el._inputNode.selectionStart).to.equal(caretIndex + 2);
        });
      });
    });
  });
