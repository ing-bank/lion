...
<Meta title="Form System/Creating a Custom Field" />

# Creating a custom field

Custom fields can be created in just a few steps. All you need is an interaction element
(like for instance a slider, a listbox or a combobox) and connect it to the [Field](?path=/docs/form-system-overview)
functionality.
...
validation support and get all the other [benefits of LionField](/?path=/docs/form-system-overview).
...
[`modelValue`](?path=/docs/form-system-modelvalue--page) of `<my-slider>`. Now the cycle is complete: the modelValue connects
