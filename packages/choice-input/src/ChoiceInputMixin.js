  ...
  class ChoiceInputMixin extends FormatMixin(superclass) {
    static get properties() {
      return {
        ...super.properties,
        /**
         * Boolean indicating whether or not this checkbox is checked.
         */
        checked: {
          type: Boolean,
          reflect: true,
    ...
    _requestUpdate(name, oldValue) {
      ...
      if (name === 'modelValue') {
        this.__syncModeToChecked({ modelValue: this.modelValue });
      } else if (name === 'checked') {
        // If there is no inputElement, this delegation will be scheduled to firstUpdated
        this.__syncCheckedToInputElement();
        this.__syncCheckedToModel();
      }
    }
    ...
    __syncModeToChecked({ modelValue }) {
      this.checked = !!modelValue.checked;
    }

    __syncCheckedToModel() {
      if (this.modelValue.checked !== this.checked) {
        this.modelValue = { ...this.modelValue, ...{ checked: this.checked } };
      }
    }
    ...
    _onModelValueChanged({ modelValue }, { modelValue: old }) {
      if (this.constructor.properties.modelValue.hasChanged(modelValue, old)) {
