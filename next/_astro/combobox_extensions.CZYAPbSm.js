...
import{_ as c}from"./preload-helper.Cj2JEybs.js";import"./node-tools_providence-analytics_overview.BxIihMx7.js";import{a as n,i as f}from"./lit-element.jD9bOQKo.js";import{x as t}from"./lit-html.DtZEZPQ5.js";import"./unsafe-html.DgrA3nwU.js";import{d}from"./dedupeMixin.6XPTJgK8.js";import{L as a}from"./LionOption.DKNRxL6v.js";import{L as l}from"./LionCombobox.BmVZfh6a.js";import{L as m}from"./LionButton.DvBSktk3.js";import{r as s}from"./renderLitAsNode.CrBoaEpB.js";const b=d(i=>class extends i{static get styles(){return[...super.styles,n`
        ...
        `}});class _ extends f{static get styles(){return[n`
        :host {
          overflow: hidden;
      ...
      `]}render(){return t` <div id="ripple"></div> `}firstUpdated(e){super.firstUpdated(e),this._ripple=this.shadowRoot.querySelector("#ripple"),this._ripple.style.cssText=`width: ${this.offsetHeight}px; height: ${this.offsetHeight}px;`,this.__onRipple=this.__onRipple.bind(this),this.addEventListener("mousedown",this.__onRipple)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener("mousedown",this.__onRipple)}__onRipple(e){this._ripple.classList.remove("animate");const o=this.getBoundingClientRect(),r={top:o.top+document.body.scrollTop,left:o.left+document.body.scrollLeft};this._ripple.style.left=`${parseInt(e.pageX-r.left,10)-this._ripple.offsetWidth/2}px`,this._ripple.style.top=`${parseInt(e.pageY-r.top,10)-this._ripple.offsetHeight/2}px`,this._ripple.classList.add("animate")}}customElements.define("md-ripple",_);const p=document.createElement("link");p.href="https://fonts.googleapis.com/css?family=Roboto:300,400,500";p.rel="stylesheet";p.type="text/css";document.head.appendChild(p);class x extends a{static styles=[...super.styles,n`
      :host {
        position: relative;
        padding: 8px;
      }
    ...
    `];onFilterMatch(e){const{innerHTML:o}=this;this.__originalInnerHTML=o,this.innerHTML=o.replace(new RegExp(`(${e})`,"i"),'<span class="md-highlight">$1</span>'),this.style.display=""}onFilterUnmatch(){this.__originalInnerHTML&&(this.innerHTML=this.__originalInnerHTML),this.style.display="none"}render(){return t`
      ${super.render()}
      <md-ripple></md-ripple>
    `}}customElements.define("md-option",x);class w extends b(l){static styles=[...super.styles,n`
      .input-group__container {
        display: flex;
        border-bottom: none;
      }
    ...
    `];constructor(){super(),this.showAllOnEmpty=!0}}customElements.define("md-combobox",w);class v extends m{static get properties(){return{value:String}}static get styles(){return n`
      :host {
        outline: none;
        position: relative;
        display: inline-flex;
    ...
    `}render(){return t` <slot name="before"></slot>
      ...
      <slot name="_button"></slot>`}}customElements.define("gh-button",v);class y extends a{static get properties(){return{category:String,default:{type:Boolean,reflect:!0}}}static get styles(){return[...super.styles,n`
        :host {
          display: flex;
          align-items: center;
          width: 100%;
    ...
    `}}customElements.define("gh-option",y);class k extends l{static get properties(){return{category:{type:String},isDesktop:{type:Boolean,reflect:!0,attribute:"is-desktop"}}}static get styles(){return[...super.styles,n`
    ...
    `}get _comboboxNode(){if(this.__comboboxNode)return this.__comboboxNode;const e=this.querySelector('[slot="input"]');if(e)return this.__comboboxNode=e,e;const o=this._overlayCtrl?.contentWrapperNode.querySelector('[slot="input"]');return o?(this.__comboboxNode=o,o):null}_inputGroupInputTemplate(){return t`
      <div class="input-group__input">
        <slot name="input"></slot>
      </div>
    `}_groupTwoTemplate(){return t` ${this._inputGroupTemplate()} ${this._feedbackTemplate()} `}get slots(){return{...super.slots,"selection-display":()=>s(t`
          <gh-button>
            <svg
              slot="before"
              text="gray"
        ...
        `)}}get _overlayInvokerNode(){return this.querySelector('[slot="selection-display"]')}get _overlayReferenceNode(){return this._overlayInvokerNode}get _categoryButtons(){return Array.from(this.shadowRoot.querySelectorAll(".gh-categories__btn[data-category]"))}constructor(){super(),this.showAllOnEmpty=!0,this.category="branches",this.selectionFollowsFocus=!1,this.__mobileDropdownComboConfig=this.config}firstUpdated(e){super.firstUpdated(e);const o=window.matchMedia("(min-width: 544px)");this.isDesktop=o.matches,o.addListener(({matches:r})=>{this.isDesktop=r})}updated(e){if(super.updated(e),e.has("category")){const o=this.category;this._categoryButtons.forEach(r=>{r.setAttribute("aria-pressed",r.getAttribute("data-category")===o?"true":"false")}),this._inputNode.placeholder=o==="branches"?"Find or create a branch...":"Find a tag",this._handleAutocompletion()}e.has("opened")&&(this._selectionDisplayNode.value=this.modelValue||"Choose a value...",this.opened?setTimeout(()=>{this._inputNode.focus()}):setTimeout(()=>{this._selectionDisplayNode.focus()},100)),e.has("isDesktop")}__handleCategory(e){this.category=e.target.getAttribute("data-category")}_textboxOnKeydown(){this.__hasSelection=this._inputNode.value.length!==this._inputNode.selectionStart}_setupOpenCloseListeners(){super._setupOpenCloseListeners(),this.__toggleOverlay=this.__toggleOverlay.bind(this),this._overlayInvokerNode.addEventListener("click",this.__toggleOverlay)}__toggleOverlay(){this.opened=!this.opened}_teardownOpenCloseListeners(){super._teardownOpenCloseListeners(),this._overlayInvokerNode.removeEventListener("click",this.__toggleOverlay)}}customElements.define("gh-combobox",k);class $ extends a{static get properties(){return{title:String,text:String,time:String,image:String,isUserText:{attribute:"is-user-text",reflect:!0,type:Boolean},isUserTextRead:{attribute:"is-user-text-read",reflect:!0,type:Boolean}}}static styles=[...super.styles,n`
      :host {
    ...
    </div>`}onFilterMatch(e){this.__originalTitle=this.title;const o=this.title.replace(new RegExp(`(${e})`,"i"),"<b>$1</b>"),r=document.createElement("div");r.innerHTML=`<span aria-label="${this.title}">${o}</span>`,[this.title]=r.children,this.style.cssText=`
    max-height: 500px;
    opacity: 1;
    `}onFilterUnmatch(){this.__originalTitle&&(this.title=this.__originalTitle),this.style.cssText=`
    max-height: 0;
    opacity: 0;
    `}}customElements.define("wa-option",$);class T extends l{static styles=[...super.styles,n`
      :host {
        font-family:
          SF Pro Text,
          SF Pro Icons,
    ...
    `];get slots(){return{...super.slots,prefix:()=>s(t`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
              d="M15.009 13.805h-.636l-.22-.219a5.184 5.184 0 0 0 1.256-3.386 5.207 5.207 0 1 0-5.207 5.208 5.183 5.183 0 0 0 3.385-1.255l.221.22v.635l4.004 3.999 1.194-1.195-3.997-4.007zm-4.808 0a3.605 3.605 0 1 1 0-7.21 3.605 3.605 0 0 1 0 7.21z"
            ></path>
          </svg>`)}}constructor(){super(),this.opened=!0,this.showAllOnEmpty=!1,this.rotateKeyboardNavigation=!1}}customElements.define("wa-combobox",T);const L=i=>class extends i{static get properties(){return{href:String,target:String}}constructor(){super(),this._nativeAnchor=document.createElement("a")}connectedCallback(){super.connectedCallback(),this.hasAttribute("role")||this.setAttribute("role","link")}firstUpdated(e){super.firstUpdated(e),this.addEventListener("click",this.__navigate),this.addEventListener("keydown",({key:o})=>{(o===" "||o==="Enter")&&this.__navigate()})}updated(e){super.updated(e),e.has("href")&&(this._nativeAnchor.href=this.href),e.has("target")&&(this._nativeAnchor.target=this.target)}__navigate(){this._nativeAnchor.click()}},z=d(L),h=t`
...
`,C=new URL("/next/_astro/googlelogo_color_272x92dp.CUA6EDD2.png",import.meta.url).href;class E extends z(a){static get properties(){return{imageUrl:{type:String}}}static get styles(){return[...super.styles,n`
        :host {
          position: relative;
          padding: 8px 16px;
          display: flex;
      ...
      `]}onFilterMatch(e){const{innerHTML:o}=this;this.__originalInnerHTML=o;const r=o.replace(new RegExp(`(${e})`,"i"),'<span class="google-option__highlight">$1</span>');this.setAttribute("aria-label",this.textContent),this.innerHTML=r,this.style.display=""}onFilterUnmatch(){this.removeAttribute("aria-label"),this.__originalInnerHTML&&(this.innerHTML=this.__originalInnerHTML),this.style.display="none"}render(){return t`
      ${this.imageUrl?t` <img class="google-option__icon" src="${this.imageUrl}" alt="" />`:t` <div class="google-option__icon">${h}</div>`}
      ${super.render()}
    `}}customElements.define("google-option",E);class B extends l{static get styles(){return[...super.styles,n`
        /** @configure FormControlMixin */
...
`,U=()=>{const i=new URL("../src/wa-combobox/assets/obama.jpeg",import.meta.url).href,e=new URL("../src/wa-combobox/assets/trump.jpeg",import.meta.url).href,o=new URL("../src/wa-combobox/assets/biden.jpeg",import.meta.url).href,r=new URL("../src/wa-combobox/assets/bush.jpeg",import.meta.url).href,u=new URL("../src/wa-combobox/assets/clinton.jpeg",import.meta.url).href;return t`
