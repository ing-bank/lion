...
async function findPackageJson(base, moduleDirs) {
  ...
  console.log({current, root, moduleDirs});
...
async function resolvePackageImports({
  ...
  console.log('resolvePackageImports');
...
async function getPackageJson(importer, pkgName, resolveOptions, moduleDirectories) {
  if (importer) {
    ...
    console.log('selfPackageJsonResult',selfPackageJsonResult);
...
async function resolveWithExportMap({
  ...
  if (importSpecifier.startsWith('#')) {
    ...
    const resolveResult = await resolvePackageImports({
      ...
      resolveId(id /* , parent*/) {
        console.log('before resolveIdClassic');
...
async function resolveWithClassic({
  ...
  console.log({importSpecifierList});

  for (let i = 0; i < importSpecifierList.length; i++) {

    console.log(importSpecifierList[i]);
...
async function resolveImportSpecifiers({
  ...
  console.log('resolveImportSpecifiers');
  try {
