...
| cacheOptions.maxCacheSize        | number   | `undefined`                                                                            | The maxiumum total size in bytes of the cache; when the cache gets larger it is truncated according to the `replacementPolicy`                  |
| cacheOptions.replacementPolicy   | function | a function that deletes the oldest cache entry until it is smaller than `maxCacheSize` | A function that reduces the cache size to below the `maxCacheSize` threshold if it is too large. In this function, `this` refers to the cache   |
...
If the cache grows larger than the `maxCacheSize` option, the cache is truncated according to the policy in the
`cacheOptions.replacementPolicy` option. The default implementation is a First-In-First-Out (FIFO) algorithm that
simply removes the oldest entries until the cache is smaller than `options.maxCacheSize`.

#### Custom replacement policy

`cacheOptions.replacementPolicy`

If you want to use a custom policy, you can provide a function with no inputs or outputs, in which the `this` value is assumed
to be the `Cache` object that is operated upon. For efficiency's sake, the `replacementPolicy` can have access to the internals of
the `Cache` object. It is recommended to use the `this.delete()` function and the `this.size` field to manipulate
the cache object.
