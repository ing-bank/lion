...
| inheritsReferenceWidth | Will align contentNode with referenceNode (or vice versa if source: 'content') for local overlays. Accepts mode string ('max', 'full', 'min', 'none') or configuration object ({ mode, source, widthOffset }). | 'max' \| 'full' \| 'min' \| 'none' \| ReferenceWidthInheritanceObject |     |     |     |
