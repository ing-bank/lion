 ...
 */
const fetchAllTestsDependencies = async (dirname) => {
  const currentFile = (await fs.promises.readFile(dirname)).toString();
 ...
 */
const getImportMap = async (resolve, dirname) => {
  const dependencies = await fetchAllTestsDependencies(dirname);
  const importMapObject = {
    imports: {},
  };
  dependencies.forEach(dependency => {
    importMapObject.imports[dependency] = resolve(dependency).split(process.env.PWD)[1];
  });
  return JSON.stringify(importMapObject);
};

export const goToPage = async (page, resolve, dirname) => {
  await page.goto(`http://localhost:8005/?importMap=${await getImportMap(resolve, dirname)}`);
};