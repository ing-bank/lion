...
import { getComponentEntry } from '../content';
...
export class UIPortalCategories extends LitElement {
  ...
  render() {
    return html`
      <div>${this._renderNavLevel({ entry: this.entry })}</div>
    `;
  }
  ...
  init() {
    ...
    this.firstDevelopmentEntry = this.webComponentEntry || this.androidComponentEntry || this.iosComponentEntry;
